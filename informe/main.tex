...
\definecolor{utecblue}{RGB}{0,51,102}
\definecolor{petroleo}{RGB}{0,105,120}
\definecolor{oxido}{RGB}{178,88,26}
\definecolor{grafito}{RGB}{85,85,85}
\definecolor{fondosuave}{RGB}{240,244,248}
...
\hypersetup{
  ...
  urlcolor=petroleo,
  citecolor=utecblue,
  pdftitle={MiniDB: Gestor de Bases de Datos Multimodal -- Entregable 1},
  pdfauthor={Brando López Carrera, Fabricio Godofredo Ladera La Torre, Edson Gustavo Guardamino Felipe, Juaquin Ernesto Huaman Garcia},
}

% ================= Títulos de sección =================
\titleformat{\section}
  {\Large\bfseries\color{utecblue}}
  {\thesection}{0.75em}{}
  [{\color{utecblue!50}\titlerule[0.9pt]}]
\titleformat{\subsection}
  {\large\bfseries\color{utecblue!90!black}}
  {\thesubsection}{0.75em}{}
\titleformat{\paragraph}[runin]
  {\normalsize\bfseries\color{petroleo}}
  {\theparagraph}{1em}{}

% ================= Encabezado y pie =================
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\itshape\color{grafito} Base de Datos II (CS2042)}
\fancyhead[R]{\small\itshape\color{grafito} MiniDB --- Entregable 1}
\fancyfoot[C]{\small\color{grafito}\thepage}
\renewcommand{\headrulewidth}{0.5pt}
\renewcommand{\headrule}{\color{utecblue!55}\hrule width\headwidth height\headrulewidth}
\setlength{\headheight}{14.5pt}

% ================= Captions y listas =================
\captionsetup{font=small, labelfont={bf,color=utecblue}, skip=6pt}
\setlist{itemsep=2pt, topsep=4pt}
\renewcommand{\arraystretch}{1.12}

% ================= Estilo común de gráficas =================
\pgfplotscreateplotcyclelist{paletadoc}{
  utecblue, mark=*\\
  petroleo, mark=square*\\
  oxido, mark=triangle*\\
  grafito, mark=diamond*\\
}
\pgfplotsset{
  estilodoc/.style={
    cycle list name=paletadoc,
    grid style={black!12},
    major tick style={black!50},
    axis line style={black!50},
    tick label style={font=\footnotesize},
    label style={font=\small},
    legend style={font=\footnotesize, draw=black!30, rounded corners=1pt, fill=white},
    line width=0.9pt,
    mark size=1.8pt,
  },
}
...
% ================= Cajas destacadas =================
\newtcolorbox{cajaformato}{
  enhanced, colback=fondosuave, colframe=utecblue,
  boxrule=0.6pt, arc=1.5mm, left=8pt, right=8pt, top=5pt, bottom=5pt,
  fontupper=\ttfamily\small
}
\newtcolorbox{cajaclave}[1]{
  enhanced, colback=petroleo!5, colframe=petroleo,
  boxrule=0.6pt, arc=1.5mm, left=8pt, right=8pt, top=10pt, bottom=6pt,
  title={#1}, fonttitle=\bfseries\small, coltitle=white,
  colbacktitle=petroleo,
  attach boxed title to top left={yshift=-2.5mm, xshift=4mm},
  boxed title style={arc=1mm, boxrule=0pt, left=5pt, right=5pt,
    top=2pt, bottom=2pt}
}
...
\begin{titlepage}
\begin{center}
{\color{utecblue}\rule{\textwidth}{2.2pt}}\\[2.5pt]
{\color{utecblue!50}\rule{\textwidth}{0.6pt}}

\vspace{1.8cm}
{\large\scshape\color{grafito} Universidad de Ingeniería y Tecnología --- UTEC}

\vspace{2.2cm}
{\large\textsc{Base de Datos II (CS2042)}}\\[0.35cm]
{\color{grafito}\large Proyecto del Curso}

\vspace{1.4cm}
{\color{utecblue}\Huge\bfseries MiniDB}\\[0.45cm]
{\LARGE Gestor de Bases de Datos Multimodal}\\[1.1cm]
{\color{petroleo}\Large\bfseries Entregable 1}

\vfill
\begin{tabular}{rl}
\textbf{Autores:} & Brando López Carrera \\[2pt]
                  & Fabricio Godofredo Ladera La Torre \\[2pt]
                  & Edson Gustavo Guardamino Felipe \\[2pt]
                  & Juaquin Ernesto Huaman Garcia \\[6pt]
\textbf{Docente:} & Percy Lovon Ramos \\[2pt]
\textbf{Ciclo:}   & 2026-II \\
\end{tabular}

\vspace{1.2cm}
{\color{grafito}\large \today}\\[0.6cm]
{\color{utecblue!50}\rule{\textwidth}{0.6pt}}\\[2.5pt]
{\color{utecblue}\rule{\textwidth}{2.2pt}}
\end{center}
\end{titlepage}
...
\begin{table}[!htb]
\centering
\caption{Layout de la página ranurada de 4 KB.}
\label{tab:slotted}
\small
\begin{tabular}{@{}l>{\raggedright\arraybackslash}p{10.6cm}@{}}
\toprule
\rowcolor{utecblue!10}\textbf{Región} & \textbf{Contenido} \\
\midrule
...
\begin{table}[!htb]
\centering
\caption{Serialización campo a campo de los tipos SQL con \texttt{struct}.}
\label{tab:struct}
\begin{tabular}{@{}lll@{}}
\toprule
\rowcolor{utecblue!10}\textbf{Tipo SQL} & \textbf{Formato struct} & \textbf{Tamaño} \\
\midrule
INT & \texttt{<i} & 4 B \\
FLOAT & \texttt{<d} & 8 B \\
BOOL & \texttt{<?} & 1 B \\
...
\begin{cajaformato}
[key\_bytes (fijo)][next\_page: I][next\_slot: H][payload]
\end{cajaformato}
...
\begin{cajaclave}{Lección de ingeniería (EXH1 $\to$ EXH2)}
...
\begin{cajaclave}{Métrica honesta por construcción}
...
\begin{table}[!htb]
\centering
\caption{Experimento 1: tiempo total de inserción masiva, en segundos.}
\label{tab:exp1}
\begin{tabular}{@{}lrrrr@{}}
\toprule
\rowcolor{utecblue!10}\textbf{Organización} & $N{=}10^3$ & $N{=}10^4$ & $N{=}5\cdot10^4$ & $N{=}10^5$ \\
\midrule
...
\end{loglogaxis}
\end{tikzpicture}
\caption{Experimento 1: escalamiento del tiempo de inserción con $N$ (escala log--log).}
\label{fig:exp1}
\end{figure}
...
\begin{table}[!htb]
\centering
\caption{Experimento 2: búsquedas puntuales sobre $N = 100\,000$ registros.}
\label{tab:exp2}
\begin{tabular}{@{}lrr@{}}
\toprule
\rowcolor{utecblue!10}\textbf{Organización} & \textbf{Reads prom.} & \textbf{Latencia prom.\ (ms)} \\
\midrule
...
\begin{table}[!htb]
\centering
\caption{Experimento 3: búsquedas por rango con selectividad variable ($N = 100\,000$).}
\label{tab:exp3}
\small
\begin{tabular}{@{}lrrrr@{}}
\toprule
\rowcolor{utecblue!10}\textbf{Organización} & \textbf{Sel.\ \%} & \textbf{Tuplas} & \textbf{Reads prom.} & \textbf{ms prom.} \\
\midrule
...
\end{semilogxaxis}
\end{tikzpicture}
\caption{Experimento 3: lecturas de bloques según la selectividad del rango (eje $x$ logarítmico).}
\label{fig:exp3}
\end{figure}
...
\begin{table}[!htb]
\centering
\caption{Experimento 4: sensibilidad del B+ Tree al tamaño de bloque $B$ ($N = 100\,000$ claves).}
\label{tab:exp4}
\small
\begin{tabular}{@{}rrrrrrr@{}}
\toprule
\rowcolor{utecblue!10}\textbf{$B$ (B)} & \textbf{Cap.\ hoja} & \textbf{Cap.\ interno} &
...
\end{tikzpicture}
\end{minipage}
\caption{Experimento 4: capacidad por tipo de nodo (izquierda) y altura del B+ Tree (derecha) según $B$.}
\label{fig:exp4}
\end{figure}
