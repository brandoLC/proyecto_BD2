...
\midrule
Cabecera (4 B) & \texttt{slot\_count} (H), \texttt{free\_start} (H) \\
...
array crece desde el inicio; \texttt{free\_start} marca el límite del área
de datos. La eliminación es \textbf{lógica}: marca el slot como muerto
...
(los separadores internos también son pares). Las hojas se enlazan con un
puntero \texttt{next}, de modo que un \emph{range search} baja una vez por
el árbol ($O(\log N)$) y luego recorre hojas contiguas. Incluye caché de
...
Resultado medido: \textbf{5 lecturas y 0.47 ms} por búsqueda (sección 7.3).
...
\begin{itemize}
  ...
  \item \texttt{SELECT} con \texttt{WHERE} (=, $<$, $\le$, $>$, $\ge$,
        \texttt{BETWEEN}, condiciones espaciales), \texttt{LIMIT/OFFSET} y
...
\midrule
Heap sin índice      & 0.008 & 0.079 & 0.412 & 0.819 \\
Heap + B+ Tree (PK)  & 0.037 & 0.442 & 2.929 & 7.075 \\
Heap + Hash Ext.\ (PK) & 0.030 & 0.356 & 2.220 & 5.245 \\
Sequential File      & 0.013 & 0.126 & 0.646 & 1.275 \\
\bottomrule
\end{tabular}
...
\addplot coordinates {(1000,0.008) (10000,0.079) (50000,0.412) (100000,0.819)};
\addplot coordinates {(1000,0.037) (10000,0.442) (50000,2.929) (100000,7.075)};
\addplot coordinates {(1000,0.030) (10000,0.356) (50000,2.220) (100000,5.245)};
\addplot coordinates {(1000,0.013) (10000,0.126) (50000,0.646) (100000,1.275)};
\legend{Heap sin índice, Heap + B+ Tree, Heap + Hash Ext., Sequential File}
\end{loglogaxis}
\end{tikzpicture}
\end{center}

Para $N = 100\,000$, las escrituras de bloques fueron: heap 1178, B+ Tree
1529, Hash 1436 y Sequential 1432 (122 132, 14 135, 19 068 y 78 458
filas/s respectivamente).

\paragraph{Análisis.}
El heap pelado es el límite inferior teórico: escribe cada página de datos
una sola vez (1178 escrituras $\approx$ $N \cdot 48$ B $/ 4096$ B) y su
throughput es constante ($\sim$120k filas/s en todo el rango), es decir
$O(1)$ amortizado por tupla. Los índices pagan el mantenimiento de la
estructura: el B+ Tree escribe $\sim$30\% más bloques y es $8.6\times$ más
...
segundo más rápido (1.28 s): gracias al puntero \texttt{tail} persistido,
...
su costo de $\sim$0.3 s por consulta).
...
\midrule
Heap sin índice (full scan) & $1178 \pm 0$    & $328.2 \pm 24.6$ \\
Heap + B+ Tree (PK)         & $6.01 \pm 0.09$ & $0.58 \pm 0.11$ \\
Heap + Hash Ext.\ (PK)      & $5 \pm 0$       & $0.47 \pm 0.09$ \\
Sequential File (binaria)   & $16.94 \pm 0.26$ & $0.55 \pm 0.11$ \\
\bottomrule
\end{tabular}
\end{center}

\paragraph{Análisis.}
Los números replican la teoría casi exactamente. El full scan lee las 1178
...
latencia (0.47 ms) — el acceso es directo por función hash, sin
...
\midrule
Heap sin índice & 0.1--25 & 100--25 000 & $1178 \pm 0$ & 300--372 \\
\midrule
Heap + B+ Tree & 0.1 & 100    & $101 \pm 1.8$   & $5.6 \pm 4.5$ \\
Heap + B+ Tree & 1   & 1 000  & $686 \pm 8.9$   & $45.2 \pm 13.9$ \\
Heap + B+ Tree & 5   & 5 000  & $1183 \pm 5.0$  & $97.0 \pm 18.6$ \\
Heap + B+ Tree & 10  & 10 000 & $1215 \pm 4.0$  & $122.9 \pm 19.8$ \\
Heap + B+ Tree & 25  & 25 000 & $1264 \pm 6.0$  & $207.3 \pm 18.0$ \\
\midrule
Sequential File & 0.1 & 100    & $18 \pm 0.6$   & $0.96 \pm 0.08$ \\
Sequential File & 1   & 1 000  & $51 \pm 1.2$   & $5.3 \pm 0.3$ \\
Sequential File & 5   & 5 000  & $204 \pm 1.5$  & $33.1 \pm 11.7$ \\
Sequential File & 10  & 10 000 & $396 \pm 1.5$  & $73.1 \pm 16.7$ \\
Sequential File & 25  & 25 000 & $972 \pm 1.5$  & $202.8 \pm 21.1$ \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{semilogxaxis}[
    ...
    legend pos=north west, grid=major, mark size=1.8pt]
\addplot coordinates {(0.1,1178) (1,1178) (5,1178) (10,1178) (25,1178)};
\addplot coordinates {(0.1,101.3) (1,685.76) (5,1183.34) (10,1214.62) (25,1264)};
...
El full scan es insensible a la selectividad (siempre 1178 páginas: hay que
...
686 páginas y gana claramente; a 5\% lee 1183, ya \emph{más} que barrer el
archivo completo. En latencia el scan nunca gana en este experimento (207 ms
del B+ Tree vs 372 ms del scan a 25\%) porque el scan además deserializa las
...
75\%) sin saltos; a 0.1\% es $5.5\times$ más económico que el B+ Tree (18 vs
101 lecturas). Es la confirmación experimental de por qué los rangos sobre
...
2048 & 204 & 145 & 3 & 718  & 1.40 & $3.01 \pm 0.10$ \\
...
\addplot coordinates {(1024,101) (2048,204) (4096,408) (8192,818)};
...
hijo por página) y la de la hoja $101 \to 204 \to 408 \to 818$ (entradas
de 10 B) — el overhead fijo de cabecera (7 B) se vuelve despreciable
...
información). El costo aparece del otro lado: la carga tarda $8.3$ s con
8 KB frente a $3.2$ s con 1 KB (serializar y escribir nodos de 8 KB por
...
\begin{itemize}
  \item Las complejidades teóricas se verificaron experimentalmente con I/O
        física real: $O(P)$ del scan (1178 lecturas constantes), $O(\log N)$
  ...
  \item El formato de persistencia importa tanto como la estructura de
        ...
        $127\times$ más lento en lectura que su versión paginada perezosa.
